...
const ImpactExplanationModal = ({ isOpen, onClose, selectedPolicies, policyIntensities }) => {
  console.log('ImpactExplanationModal render:', { isOpen, selectedPolicies, policyIntensities });
  ...
  }).filter(p => p && p.intensity !== 50) // Only show policies that have been adjusted
  ...
  try {
    ...
    console.error('Error calculating policy pairs:', error);
  }
  ...
  try {
    ...
    console.error('Error rendering ImpactExplanationModal:', error);
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
          <div className="text-center">
            <h2 className="text-xl font-bold text-red-600 mb-4">Error Loading Modal</h2>
            <p className="text-slate-600 mb-4">There was an error displaying the impact explanation. Please try again.</p>
            <button
              onClick={onClose}
...
function AppContent() {
  ...
  const [policyIntensities, setPolicyIntensities] = useState({
    DATA_ANALYTICS: 0,
    PROTECT_STD: 0,     // Starting point: 0%
    PD_FUNDS: 0,        // Starting point: 0%
    EDUC_AUTONOMY: 0,
    AI_INTEGRATION: 0,   // Starting point: 0%
    DIGITAL_CITIZEN: 0, // Starting point: 0%
    COMM_INPUT: 0,
    IMPACT_REP_STD: 0,
    LOCAL_JOB_ALIGN: 0,
    INTEROP_STD: 0,
    INFRA_INVEST: 0,    // Starting point: 0%
    ACCESS_STD: 0,      // Starting point: 0%
    STATE_FED_PART: 0,  // Starting point: 0%
    INNOV_SANDBOX: 0,   // Starting point: 0%
    MODEL_EVAL_STD: 0   // Starting point: 0%
  })
  ...
  const handleReset = () => {
    ...
    // Reset all policy intensities to 0% starting point
  ...
  const getPolicyResources = (policyId) => {
    const resources = {
      ...
      'AI_INTEGRATION': '• Artificial Intelligence and the Future of Teaching and Learning: https://www.ed.gov/sites/ed/files/documents/ai-report/ai-report.pdf\n• Shaping the Future of Learning: The Role of AI in Education 4.0: https://www3.weforum.org/docs/WEF_Shaping_the_Future_of_Learning_2024.pdf',
                                ...
                                <input
                                  ...
                                  style={
                                    policyIntensities[policy.id] !== undefined ? {
                                      background: (() => {
                                        const value = policyIntensities[policy.id];
                                        return `linear-gradient(to right, #ea580c 0%, #ea580c ${value}%, #e2e8f0 ${value}%, #e2e8f0 100%)`;
                                      })()
                                    } : {
                                      background: '#e2e8f0'
                                    }
                                ...
                                <input
                                  ...
                                  style={
                                    policyIntensities[policy.id] !== undefined ? {
                                      background: (() => {
                                        const value = policyIntensities[policy.id];
                                        return `linear-gradient(to right, #dc2626 0%, #dc2626 ${value}%, #e2e8f0 ${value}%, #e2e8f0 100%)`;
                                      })()
                                    } : {
                                      background: '#e2e8f0'
                                    }
